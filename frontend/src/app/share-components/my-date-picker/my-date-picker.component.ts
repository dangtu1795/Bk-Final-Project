...
import {IMyDayModel} from "./interfaces/my-day-model.interface";

// webpack1_
// declare var require: any;
// const myDpStyles: string = require();
// const myDpTpl: string = require();
// webpack2_
...
export class MyDatePicker implements OnChanges, ControlValueAccessor {
  ...
  @Output() dayChanged: EventEmitter<IMyDayModel> = new EventEmitter<IMyDayModel>();
  ...
  setOptions(): void {
    ...
    if (this.options.showWeekDayOnly) {
      this.opts.selectorHeight = "55px";
    }
  ...
  onWeekDayClicked(event, day) {
    if (this.opts.showWeekDayOnly) {
  ...
  parseOptions(): void {
    ...
    if (this.dayIdx !== -1) {
      ...
      for (let i = 0; i < weekDays.length; i++) {
        this.weekDays.push({
          label: this.opts.dayLabels[weekDays[idx]],
          dayOfWeek: idx
        });
  ...
  setFocusToInputBox(): void {
    if (!this.opts.inline) {
      setTimeout(() => {
        this.inputBoxEl && this.inputBoxEl.nativeElement.focus();
      });
    }
  }
